...
generate_data <- function(m, 
  ...
  outlier_to_clust <- sample(1:num_clust, size=num_outliers, replace=TRUE)
...
var everything
```{r}
prefix <- "clustered_outliers_small"
prep_output(prefix)
...
for (j in 50:99) {
  n <- sample(3:25, size=1) 
  m <- sample((2*n):100, size=1)# so m/n is above 2
  print("new")
  print(m)
  print(n)
  my_data <- generate_data( 
