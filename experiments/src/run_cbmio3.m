...
% TSEstar after final PCA, TSE after final PCA, gamma after final PCA
% MIO time,  CB time, total time
    fprintf(out_file, "%s,%d,%d,%d,%d,%d,%s,%f,%s,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%f,%d,%d,%d,%d,%f,%f,%f,%d,%d\n", datafname, iteration, m, n, m_normal, q, formulation, result.runtime, result.status, output.gamma(4,1), result.objbound, output.TSEstar(1,1), output.TSE(1,1), output.gamma(1,1), output.TSEstar(2,1), output.TSE(2,1), output.MIOgamma, output.TSEstar(3,1), output.TSE(3,1), output.gamma(3,1), output.TSEstar(4,1), output.TSE(4,1), output.gamma(4,1),output.closeAll(1,1),output.closeAll(2,1),output.closeAll(3,1),output.closeAll(4,1),output.MIOTime, output.CBregTime, output.solTime,output.cbq,output.outfinderq); 
else 
