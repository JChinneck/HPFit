...
if dep_var == true
    X = [X(:,1) ones(m,1) X(:,2:n)]; % add column of 1s after the response column
else
...
[beta_star, f_beta_star] = algorithm1(X, beta_star, q, dep_var) % use Algorithm 2 solution as input to Algorithm 1
...
if dep_var == true % get error along response direction
    tot_err = sum(dist(1:m_normal,1).*dist(1:m_normal,1)); % sum of squared errors
    dist_sort = sort(dist(:,1).*dist(:,1)); % sorted absolute distances
    alg3_lts = sum(dist_sort(1:m_normal,1)); % sum of m smallest absolute distances 
else % get orthogonal error
    gradLen = norm(beta_star(2:n,1));
    edist = abs(dist/gradLen);
    tot_err = sum(edist(1:m_normal,1).*edist(1:m_normal,1)); % sum of squared errors
    dist = edist(:,1).*edist(:,1); % squared orthogonal distances
    dist_sort = sort(dist(:,1)); % sorted squared distances
    alg3_lts = sum(dist_sort(1:m_normal,1)) % sum of m smallest squared distances
end

out_fname = strcat(resloc, "/alg3", init_method, "i", int2str(iteration), ".csv")
disp(out_fname)
